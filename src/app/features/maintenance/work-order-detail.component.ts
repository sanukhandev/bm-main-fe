...
import { InventoryItem, Vendor, WorkOrder, WorkOrderPayment } from '../../shared/models/maintenance.models';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, RouterLink, BmStatusBadgeComponent, BmLoadingStateComponent, BmSpinnerComponent],
  template: `
    <div class="max-w-5xl mx-auto space-y-6">
      @if (isLoading()) {
        ...
        <div class="flex items-center justify-between gap-4">
          <div>
            <a routerLink="/app/maintenance/work-orders" class="text-xs text-emerald-700 font-semibold hover:underline">← Back to Work Orders</a>
            <h1 class="text-3xl font-bold text-slate-900 mt-2 tracking-tight">{{ item.work_order_no }}</h1>
          </div>
          <div class="flex gap-2">
            <button type="button" class="bm-btn bm-btn-secondary" (click)="paymentOpen.set(true)">+ Add Payment</button>
        ...
        @if (editing()) {
          <form [formGroup]="form" (ngSubmit)="save()" class="bm-card p-6 space-y-4">
            ...
            <textarea class="bm-input" rows="4" formControlName="description" placeholder="Description"></textarea>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <input class="bm-input" type="number" min="0" step="0.01" formControlName="service_charge" placeholder="Service charge">
            </div>
            ...
            <button class="bm-btn bm-btn-primary" [disabled]="form.invalid || saving()">
              @if (saving()) { <bm-spinner size="sm" color="white" label="Saving..."></bm-spinner> } @else { Save Changes }
            </button>
          </form>
        } @else {
          <div class="bm-card p-6 grid grid-cols-1 md:grid-cols-2 gap-5 text-sm">
            <div><div class="text-slate-500 font-medium">Property</div><div class="font-semibold text-slate-900 mt-1">{{ item.property?.name || item.property?.property_code || '—' }}</div></div>
            <div><div class="text-slate-500 font-medium">Vendor</div><div class="font-semibold text-slate-900 mt-1">{{ item.vendor?.name || '—' }}</div></div>
            <div><div class="text-slate-500 font-medium">Title</div><div class="font-semibold text-slate-900 mt-1">{{ item.title }}</div></div>
            <div><div class="text-slate-500 font-medium">Status</div><div class="mt-1"><bm-status-badge [status]="item.status"></bm-status-badge></div></div>
            <div><div class="text-slate-500 font-medium">Priority</div><div class="mt-1"><bm-status-badge [status]="item.priority"></bm-status-badge></div></div>
            <div><div class="text-slate-500 font-medium">Service Charge</div><div class="font-semibold text-slate-900 mt-1">AED {{ item.service_charge }}</div></div>
            <div class="md:col-span-2"><div class="text-slate-500 font-medium">Description</div><p class="mt-1 text-slate-700 whitespace-pre-wrap">{{ item.description || '—' }}</p></div>
          </div>
        }
        ...
        <div class="bm-card overflow-x-auto">
          ...
          <table class="w-full text-left text-sm">
            <thead class="bg-slate-50 text-slate-500 uppercase text-xs">
              <tr><th class="p-4">Type</th><th class="p-4">Particulars</th><th class="p-4">Inventory</th><th class="p-4">Qty</th><th class="p-4">Unit Cost</th><th class="p-4">Total</th></tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              @for (line of item.lines || []; track line.id) {
                <tr>
                  ...
                  <td class="p-4 font-semibold tabular-nums">AED {{ (toNumber(line.quantity) * toNumber(line.unit_cost)).toFixed(2) }}</td>
                </tr>
              } @empty {
                <tr><td colspan="6" class="p-8 text-center text-slate-500 font-medium">No line items.</td></tr>
              }
            </tbody>
          </table>
        </div>
        ...
        <div class="bm-card overflow-x-auto">
          <div class="p-5 border-b border-slate-100 flex justify-between items-center">
            ...
            <span class="text-xs text-slate-500 font-normal">Receipts and vouchers are generated when posted</span>
          </div>
          <table class="w-full text-left text-sm">
            <thead class="bg-slate-50 text-slate-500 uppercase text-xs">
              <tr><th class="p-4">Direction</th><th class="p-4">Particulars</th><th class="p-4">Category</th><th class="p-4">Mode</th><th class="p-4">Amount</th><th class="p-4">Status</th><th class="p-4">Action</th></tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              @for (payment of payments(); track payment.id) {
                <tr>
                  <td class="p-4"><bm-status-badge [status]="payment.direction"></bm-status-badge></td>
                  <td class="p-4">{{ payment.particulars }}</td>
                  <td class="p-4">{{ payment.category }}</td>
                  <td class="p-4"><bm-status-badge [status]="payment.payment_mode"></bm-status-badge></td>
                  ...
                  <td class="p-4">
                    <button type="button" class="text-emerald-700 text-xs font-semibold hover:underline" [disabled]="payment.status === 'paid'" (click)="setPaymentStatus(payment, 'paid')">
                      Mark Paid
                    </button>
                  </td>
                </tr>
              } @empty {
                <tr><td colspan="7" class="p-8 text-center text-slate-500 font-medium">No payments added.</td></tr>
              }
            </tbody>
          </table>
        </div>
      ...
      @if (paymentOpen()) {
        <div class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <form [formGroup]="paymentForm" (ngSubmit)="savePayment()" class="bm-card w-full max-w-xl p-6 space-y-4 shadow-2xl">
            <div class="flex justify-between items-center pb-3 border-b border-slate-100">
              <h2 class="text-lg font-bold text-slate-900">Add Work Order Payment</h2>
              <button type="button" class="text-xl font-bold text-slate-400 hover:text-slate-600" (click)="paymentOpen.set(false)">×</button>
            </div>
            ...
            <div class="flex justify-end gap-2 pt-2 border-t border-slate-100">
              <button type="button" class="bm-btn bm-btn-secondary" (click)="paymentOpen.set(false)">Cancel</button>
              <button class="bm-btn bm-btn-primary" [disabled]="paymentForm.invalid || saving()">
                @if (saving()) { <bm-spinner size="sm" color="white" label="Saving..."></bm-spinner> } @else { Save Payment }
              </button>
            </div>
          </form>
        </div>
