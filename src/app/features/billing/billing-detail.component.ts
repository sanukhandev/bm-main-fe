...
@Component({
  ...
  template: ` <div class="max-w-5xl mx-auto space-y-6">
    @if (error()) {
      <div class="bm-card p-4 text-rose-700">{{ error() }}</div>
    }
    @if (record()) {
      <div class="flex flex-wrap justify-between gap-4">
        <div>
          <a [routerLink]="['/app/billing', type()]" class="text-sm text-emerald-700">← Back</a>
          <h1 class="text-3xl font-bold text-slate-900 mt-2">{{ number(record()!) }}</h1>
          <p class="text-sm text-slate-500">{{ record()!.title }} · {{ record()!.status }}</p>
        </div>
        <div class="flex gap-2">
          ...
          @if (type() === 'quotations' && record()!.status !== 'converted') {
            <button class="bm-btn bm-btn-primary" (click)="convert()">Convert to Invoice</button>
          }
        </div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bm-card p-4">
          ...
          <div class="text-xl font-bold">AED {{ money(record()!.subtotal) }}</div>
        </div>
        <div class="bm-card p-4">
          <div class="text-xs text-slate-500">Tax</div>
          <div class="text-xl font-bold">AED {{ money(record()!.tax_amount) }}</div>
        </div>
        <div class="bm-card p-4 bg-emerald-50">
          <div class="text-xs text-emerald-700">Total</div>
          <div class="text-xl font-bold text-emerald-800">
            AED {{ money(record()!.total_amount) }}
          </div>
        </div>
      </div>
      <div class="bm-card overflow-x-auto">
        <h2 class="p-5 font-semibold border-b">Line Items</h2>
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y">
            @for (line of record()!.lines || []; track line.id) {
              <tr>
                <td class="p-4">{{ line.particulars }}</td>
                <td class="p-4">{{ line.quantity }}</td>
                <td class="p-4">AED {{ money(line.unit_price) }}</td>
                <td class="p-4 text-right">AED {{ money(line.line_total) }}</td>
              </tr>
            }
          </tbody>
        </table>
      </div>
      <div class="bm-card p-5 space-y-4">
        <div class="flex flex-wrap justify-between gap-3">
          <div>
            <h2 class="font-semibold">Payment Lines</h2>
            <p class="text-xs text-slate-500 mt-1">
              Posted lines generate Accounts receipts or vouchers.
            </p>
          </div>
          <button
          ...
          >
            {{ showPaymentForm() ? 'Close' : 'Add Payment Line' }}
          </button>
        </div>
        @if (showPaymentForm()) {
          <form
            (ngSubmit)="addPayment()"
            class="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 rounded-xl bg-slate-50"
          >
            <select class="bm-input" [(ngModel)]="payment.direction" name="direction">
              <option value="inward">Inward</option>
              <option value="outward">Outward</option></select
            ><input
              class="bm-input"
            ...
            <textarea
              class="bm-input md:col-span-3"
              [(ngModel)]="payment.remarks"
              name="remarks"
              placeholder="Remarks"
            ></textarea
            ><button class="bm-btn bm-btn-primary">Save Payment Line</button>
          </form>
        }
          ...
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y">
              @for (line of record()!.payments || []; track line.id) {
                <tr>
                  <td class="p-3 capitalize">{{ line.direction }}</td>
                  <td class="p-3">{{ line.particulars }}</td>
                  <td class="p-3 capitalize">{{ line.payment_mode.replace('_', ' ') }}</td>
                  <td class="p-3">AED {{ money(line.amount) }}</td>
                  <td class="p-3 capitalize">{{ line.status }}</td>
                  <td class="p-3">
                    @if (line.receipt) {
                      <a
                        [routerLink]="receiptRoute(line.receipt.direction)"
                        class="text-blue-700 font-semibold text-xs mr-3 hover:underline"
                        [attr.aria-label]="'Open receipt ' + line.receipt.document_no"
                      >
                        Receipt {{ line.receipt.document_no }}
                      </a>
                    }
                    <button
                ...
                <tr>
                  <td colspan="6" class="p-4 text-center text-slate-500">No payment lines.</td>
                </tr>
              }
            </tbody>
          </table>
      ...
      <div class="bm-card p-8 text-center text-slate-500">Loading...</div>
    }
  </div>`,
})
