    ...
    @if (isLoading()) {
      ...
      <div class="agreement-screen max-w-[1360px] w-full mx-auto px-4 sm:px-6 lg:px-8 pb-16">
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p class="text-xs lg:text-sm text-slate-500 font-normal mt-1">
                {{ tenantName() }} · {{ primaryPropertyName() }} ·
            ...
            <div class="flex items-center gap-2.5 flex-wrap">
              @if (agreement()!.status === 'draft') {
                ...
                <a
                  [routerLink]="['/app/tenant-agreements', agreement()!.id, 'edit']"
        ...
        <div class="bg-white rounded-[22px] p-6 lg:p-8 border border-slate-200/90 shadow-xs mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 pb-8 border-b border-slate-100">
                ...
                @if (tenantCustomer()) {
                  ...
                  @if (tenantCustomer()?.phone) {
                    ...
                    >
                      ...
                      <span>{{ tenantCustomer()?.phone }}</span>
                    </div>
                  }
                }
              </div>
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span>Leased Property Asset</span>
              </div>
              <div>
                @if (primaryProperty()) {
                  ...
                  <div class="text-xs text-slate-500 mt-1 tabular-nums">
                    <span class="text-slate-400">Code:</span>
                    {{ primaryProperty()!.property_code }} · Unit
                    {{ primaryProperty()!.unit_number }}
                  </div>
              ...
              @if (primaryProperty()) {
                <div class="pt-2 flex items-center justify-between">
                  <a
                    [routerLink]="['/app/properties', primaryProperty()!.id]"
                    class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700 hover:text-emerald-800 transition"
                  >
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span>Lease Validity Period</span>
              </div>
              <div>
                <div
          ...
          <!-- FINANCIAL SUMMARY PANEL (Clean text strip with no colored card box) -->
          <div>
            <div
            ...
            <div class="grid grid-cols-2 md:grid-cols-6 gap-6 text-xs">
              <div>
                ...
                <div
                  class="whitespace-nowrap tabular-nums font-semibold tracking-tight text-xl text-slate-900"
                >
              ...
              <div>
                ...
                <div
                  class="whitespace-nowrap tabular-nums font-semibold tracking-tight text-xl text-emerald-700"
                >
              ...
              <div>
                ...
                <div
                  class="whitespace-nowrap tabular-nums font-semibold tracking-tight text-xl text-amber-700"
                >
                  <span class="text-xs text-amber-600 uppercase font-medium mr-1">AED</span>
        ...
        <div class="bg-white rounded-[22px] p-6 lg:p-8 border border-slate-200/90 shadow-xs mb-8">
          <div class="flex items-center justify-between pb-4 mb-6 border-b border-slate-100">
            <div>
              ...
              <p class="text-xs text-slate-500 font-normal mt-0.5">
                ...
                {{
                  (agreement()!.installments?.length || 0) - getPaidCount(agreement()!.installments)
                }}
                remaining
              </p>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6 pb-6 border-b border-slate-100">
            <div>
              ...
              <div class="text-xl font-bold text-slate-900 tabular-nums">
                <span class="text-xs text-slate-400 font-medium uppercase mr-1">AED</span>
                <span>{{ formatAmount(getScheduleTotal(agreement()!.installments)) }}</span>
              </div>
            </div>
            <div>
              <div class="text-xs font-semibold text-emerald-700 uppercase tracking-wider mb-1">
                Total Paid
              </div>
              <div class="text-xl font-bold text-emerald-700 tabular-nums">
                <span class="text-xs text-emerald-600 font-medium uppercase mr-1">AED</span>
                <span>{{ formatAmount(getPaidTotal(agreement()!.installments)) }}</span>
              </div>
            </div>
            <div>
              <div class="text-xs font-semibold text-amber-700 uppercase tracking-wider mb-1">
                Outstanding Balance
              </div>
              <div class="text-xl font-bold text-amber-700 tabular-nums">
                <span class="text-xs text-amber-600 font-medium uppercase mr-1">AED</span>
                <span>{{ formatAmount(getOutstandingTotal(agreement()!.installments)) }}</span>
              </div>
            </div>
          </div>
                      ...
                      <div class="flex items-center justify-end gap-1.5">
                        @if (isPaidOrVoid(item)) {
                          ...
                          <button
                            ...
                            class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200/60 inline-flex items-center justify-center transition shadow-2xs disabled:opacity-40"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              class="h-4 w-4"
                          ...
                          @if (item.status !== 'defaulted') {
                            <button
                              ...
                              class="w-8 h-8 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200/60 inline-flex items-center justify-center transition shadow-2xs disabled:opacity-40"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                class="h-4 w-4"
            ...
            @if (agreement()!.notes) {
              <div class="bg-white rounded-[22px] p-6 border border-slate-200/90 shadow-xs">
                <h3 class="text-sm font-semibold text-slate-900 tracking-tight mb-2">
                  Lease Notes & Special Conditions
                </h3>
...
export class TenantAgreementDetailComponent implements OnInit {
    ...
    this.api
      ...
      .subscribe({
        next: (res) => {
          ...
          this.router.navigate(['/app/tenant-agreements', res.data.id]);
        },
        error: (err) => {
          this.isActioning.set(false);
  ...
  openVoucherModal(item: AgreementInstallment): void {
    ...
    if (!agr || !item) return;
    const partyName = this.tenantName();
    const customerCode = this.tenantCustomer()?.customer_code || '';

    const receiptObj = item.receipt || (item as any).payment_receipt || null;
    const docNo =
      receiptObj?.document_no ||
      (item as any).document_no ||
      `REC-${agr.agreement_no}-${item.installment_no || 1}`;
    const amountVal =
    ...
    const tx: AccountTransaction = {
      ...
      remarks:
        item.notes ||
        `Installment #${item.installment_no} for Tenant Agreement ${agr.agreement_no}`,
