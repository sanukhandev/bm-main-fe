...
export class TenantAgreementDetailComponent implements OnInit {
  ...
  addExtraPayment(): void { const id = this.agreement()?.id; const category = prompt('Payment line category (commission/security)'); const amount = category ? Number(prompt('Amount') || 0) : 0; if (!id || !category || amount <= 0) return; this.api.addAdditionalPayment(id, { direction: 'inward', category, amount }).subscribe({ next: () => this.loadAgreement(), error: (err) => alert(err.message || 'Unable to add payment line.') }); }
}
