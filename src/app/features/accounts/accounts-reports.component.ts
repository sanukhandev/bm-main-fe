...
@Component({
  ...
  template: `
    <bm-page-header
      ...
      subtitle="Backend-calculated branch financial summaries"
    ...
    @if (loading()) {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section class="bm-card overflow-hidden">
          <div class="p-4 border-b font-semibold">Daily Cash Movement</div>
          <table class="w-full text-left text-xs">
            <thead class="bg-slate-50 text-slate-500">
              <tr>
                <th class="p-4">Date</th>
                <th class="p-4">Inward</th>
                <th class="p-4">Outward</th>
                <th class="p-4">Net</th>
              </tr>
            </thead>
            <tbody class="divide-y">
              @for (row of movement(); track row.date) {
                <tr>
                  ...
                  <td class="p-4 text-emerald-700">AED {{ row.inward }}</td>
                  <td class="p-4 text-rose-700">AED {{ row.outward }}</td>
                  <td class="p-4 font-semibold">AED {{ row.net }}</td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <section class="bm-card overflow-hidden">
          <div class="p-4 border-b font-semibold">Payment Mode Summary</div>
          <table class="w-full text-left text-xs">
            <thead class="bg-slate-50 text-slate-500">
              <tr>
            ...
            <tbody class="divide-y">
              @for (row of modes(); track row.payment_mode + row.direction) {
                <tr>
                  ...
                  <td class="p-4"><bm-status-badge [status]="row.direction"></bm-status-badge></td>
                  <td class="p-4">{{ row.count }}</td>
                  <td class="p-4 font-semibold">AED {{ row.amount }}</td>
                </tr>
              }
            </tbody>
          </table>
...
export class AccountsReportsComponent implements OnInit {
  ...
  private fail(err: { message?: string }): void {
    this.error.set(err.message || 'Unable to load reports.');
    this.loading.set(false);
  }
}
