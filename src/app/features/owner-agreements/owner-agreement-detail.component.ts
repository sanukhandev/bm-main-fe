...
export class OwnerAgreementDetailComponent implements OnInit {
  ...
  addExtraPayment(): void { const id = this.agreement()?.id; const direction = prompt('Direction: inward or outward', 'outward') as 'inward' | 'outward'; const category = prompt('Category (commission/security)'); const particulars = category ? prompt('Particulars') : null; const amount = particulars ? Number(prompt('Amount') || 0) : 0; const dueDate = particulars ? prompt('Due date (YYYY-MM-DD)', new Date().toLocaleDateString('en-CA')) : null; const mode = dueDate ? prompt('Payment mode: cash, cheque, or bank_transfer', 'cash') : null; const terms = mode ? prompt('Additional payment terms (optional)') || undefined : undefined; if (!id || !['inward', 'outward'].includes(direction) || !category || !particulars || amount <= 0 || !dueDate || !mode || (mode !== 'cash' && mode !== 'cheque' && mode !== 'bank_transfer')) return; this.api.addAdditionalPayment(id, { direction, category, particulars, amount, due_date: dueDate, payment_mode: mode, terms }).subscribe({ next: () => this.loadAgreement(), error: (err) => alert(err.message || 'Unable to add payment line.') }); }
}
