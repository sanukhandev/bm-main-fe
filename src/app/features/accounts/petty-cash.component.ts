...
@Component({ selector: 'bm-petty-cash', standalone: true, imports: [CommonModule, RouterLink, ReactiveFormsModule, BmPageHeaderComponent, BmLoadingStateComponent, BmErrorStateComponent], template: `
  <bm-page-header title="Petty Cash Daybook" subtitle="Daily cash income and expenses"><button type="button" (click)="entryOpen.set(!entryOpen())" class="bm-btn bm-btn-primary text-xs">{{ entryOpen() ? 'Close Entry' : 'Create Daybook Entry' }}</button><a routerLink="/app/accounts/dashboard" class="bm-btn bm-btn-secondary text-xs">Accounts Dashboard</a></bm-page-header>
  @if (entryOpen()) { <form [formGroup]="entryForm" (ngSubmit)="save()" class="bm-card p-4 mb-5 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
...
export class PettyCashComponent implements OnInit {
  private api = inject(AccountsApiService); private fb = inject(FormBuilder); book = signal<PettyCashDaybookResponse | null>(null); rows = signal<AccountTransaction[]>([]); loading = signal(true); saving = signal(false); entryOpen = signal(true); error = signal<string | null>(null);
