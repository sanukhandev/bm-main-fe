              ...
              @for (agr of agreements(); track agr.id) {
                <tr class="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td class="py-3.5 px-4 text-right space-x-2">
                    <a [routerLink]="['/app/tenant-agreements', agr.id]" class="text-emerald-700 hover:text-emerald-900 font-medium text-xs">
                      View
                    </a>

                    @if (agr.status === 'draft' || agr.status === 'pending_approval') {
                      <a [routerLink]="['/app/tenant-agreements', agr.id, 'edit']" class="text-slate-600 hover:text-slate-900 font-medium text-xs">
                        Edit
                      </a>
                    }
