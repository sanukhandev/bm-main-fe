    ...
    @if (isLoading()) {
      ...
      <div class="bm-card p-8 md:p-12 max-w-4xl mx-auto space-y-8 bg-white border border-[#DDE3DF]">
        ...
        <div>
          ...
          <div class="overflow-x-auto border border-slate-200 rounded-xl"><table class="w-full text-left text-xs"><thead class="bg-slate-50 text-slate-500 uppercase"><tr><th class="p-3">#</th><th class="p-3">Due Date</th><th class="p-3">Particulars</th><th class="p-3">Mode</th><th class="p-3">Scheduled</th><th class="p-3">Paid</th><th class="p-3">Balance</th><th class="p-3">Status</th><th class="p-3">Action</th></tr></thead><tbody class="divide-y divide-slate-100">@for (item of agreement()!.installments || []; track item.id) { <tr><td class="p-3">{{ item.installment_no }}</td><td class="p-3">{{ item.due_date }}</td><td class="p-3">{{ item.notes || 'Rent installment' }}</td><td class="p-3 capitalize">{{ item.payment_mode.replace('_', ' ') }}</td><td class="p-3">AED {{ item.amount }}</td><td class="p-3">AED {{ item.paid_amount }}</td><td class="p-3 font-semibold">AED {{ item.balance }}</td><td class="p-3 capitalize">{{ item.status.replace('_', ' ') }}</td><td class="p-3 whitespace-nowrap"><div class="flex items-center gap-1.5"><button type="button" [disabled]="installmentProcessingId() === item.id || item.status === 'paid'" (click)="setInstallmentStatus(item.id, 'paid')" title="Mark as Paid" aria-label="Mark as Paid" class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200/60 inline-flex items-center justify-center transition shadow-2xs disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-emerald-50"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg></button><button type="button" [disabled]="installmentProcessingId() === item.id || item.status === 'paid' || item.status === 'defaulted'" (click)="setInstallmentStatus(item.id, 'defaulted')" title="Mark as Defaulted" aria-label="Mark as Defaulted" class="w-8 h-8 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200/60 inline-flex items-center justify-center transition shadow-2xs disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-rose-50"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg></button></div></td></tr> } @empty { <tr><td colspan="9" class="p-5 text-center text-slate-500">No payment schedule found.</td></tr> }</tbody></table></div>
        </div>
      </div>

      @if (paymentLineOpen()) { <div class="fixed inset-0 z-50 bg-slate-900/40 flex items-center justify-center p-4"><div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl p-6"><div class="flex items-center justify-between mb-5"><h2 class="text-lg font-semibold">Add Payment Line</h2><button type="button" (click)="paymentLineOpen.set(false)" class="text-slate-500 text-xl">×</button></div><div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs"><label>Direction<select [(ngModel)]="paymentLine.direction" class="bm-input mt-1"><option value="outward">Outward</option><option value="inward">Inward</option></select></label><label>Category<input [(ngModel)]="paymentLine.category" class="bm-input mt-1" placeholder="Commission / security"></label><label>Particulars<input [(ngModel)]="paymentLine.particulars" class="bm-input mt-1"></label><label>Amount<input [(ngModel)]="paymentLine.amount" type="number" min="0.01" step="0.01" class="bm-input mt-1"></label><label>Due Date<input [(ngModel)]="paymentLine.due_date" type="date" class="bm-input mt-1"></label><label>Payment Mode<select [(ngModel)]="paymentLine.payment_mode" class="bm-input mt-1"><option value="cash">Cash</option><option value="cheque">Cheque</option><option value="bank_transfer">Bank Transfer</option></select></label><label class="md:col-span-2">Terms<textarea [(ngModel)]="paymentLine.terms" rows="3" class="bm-input mt-1"></textarea></label></div><div class="flex justify-end gap-3 mt-6"><button type="button" (click)="paymentLineOpen.set(false)" class="bm-btn bm-btn-secondary text-xs">Cancel</button><button type="button" (click)="saveExtraPayment()" class="bm-btn bm-btn-primary text-xs">Save Payment Line</button></div></div></div> }
...
export class OwnerAgreementDetailComponent implements OnInit {
  ...
  paymentLine = { direction: 'outward' as 'inward' | 'outward', category: '', particulars: '', amount: 0, due_date: new Date().toLocaleDateString('en-CA'), payment_mode: 'cash' as 'cash' | 'cheque' | 'bank_transfer', terms: '' };
  ...
  addExtraPayment(): void { this.paymentLineOpen.set(true); }
  saveExtraPayment(): void { const id = this.agreement()?.id; if (!id || !this.paymentLine.category || !this.paymentLine.particulars || this.paymentLine.amount <= 0 || !this.paymentLine.due_date) return; this.api.addAdditionalPayment(id, this.paymentLine).subscribe({ next: () => { this.paymentLineOpen.set(false); this.loadAgreement(); }, error: (err) => alert(err.message || 'Unable to add payment line.') }); }
}
