  ...
  template: `
    @if (isLoading()) {
      ...
      <div class="max-w-[1360px] w-full mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <div class="mb-6">
          <a routerLink="/app/owner-agreements" class="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 hover:text-emerald-800 transition mb-3">
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p class="text-xs lg:text-sm text-slate-500 font-normal mt-1">
                {{ ownerName() }} · {{ propertiesList().length > 0 ? (propertiesList()[0].name || propertiesList()[0].property_code) : 'N/A' }} · {{ formatDate(agreement()!.start_date) }} – {{ formatDate(agreement()!.end_date) }}
              </p>
            </div>
        ...
        <div class="bg-white rounded-[22px] p-6 lg:p-8 border border-slate-200/90 shadow-xs mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/70 flex flex-col justify-between">
              <div>
                ...
                @if (ownerCustomer()) {
                  <div class="text-xs text-slate-500 mt-1 tabular-nums">Code: {{ ownerCustomer()?.customer_code }}</div>
                  <div class="text-xs text-slate-500 tabular-nums">{{ ownerCustomer()?.phone || '—' }}</div>
                }
              </div>
              @if (ownerCustomer()) {
                <div class="pt-4 mt-2 border-t border-slate-200/60">
                  <a [routerLink]="['/app/customers', ownerCustomer()?.id]" class="text-xs font-semibold text-emerald-700 hover:text-emerald-800 transition flex items-center gap-1">
            ...
            <!-- 2. Covered Properties -->
            <div class="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/70 flex flex-col justify-between">
              <div>
                <div class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">
                  Managed Property Asset(s)
                </div>
              ...
              @if (propertiesList().length > 0) {
                <div class="pt-4 mt-2 border-t border-slate-200/60 flex items-center justify-between">
                  <a [routerLink]="['/app/properties', propertiesList()[0].id]" class="text-xs font-semibold text-emerald-700 hover:text-emerald-800 transition flex items-center gap-1">
          ...
          <div class="p-6 rounded-2xl bg-emerald-50/60 border border-emerald-100/80">
            ...
            <div class="grid grid-cols-2 md:grid-cols-6 gap-6 text-xs">
              <div>
                <div class="text-slate-500 font-medium mb-1">Total Contract Value</div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            @if (agreement()!.notes) {
              <div class="bg-white rounded-[22px] p-6 border border-slate-200/90 shadow-xs">
                <h3 class="text-sm font-semibold text-slate-900 tracking-tight mb-2">Additional Terms & Notes</h3>
